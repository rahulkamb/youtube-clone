import React from 'react'
import './containt.css'
import VideoCard from '../Maincontaint/videoCard';

import ProfileLogo from '../Images/profilePic.jpg'

function containt({isContentOpen}) {
  return (
    <div className={`content${isContentOpen?'NonActive':'Active'}`}>
        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

        <VideoCard isContentOpen={isContentOpen}
            image='https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw'
            avtar={ProfileLogo}
            title="😍 Let's Build a WhatsApp clone with REACT JS for Beginners!"
            channelName='Start with React'
            views='532M views'
            timeStamp='Streamed 3 months ago'
        />

      {/* <div className='content_thumbnail'>
        <img alt="thumnail"  src="https://i.ytimg.com/vi/pUxrDcITyjg/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&amp;rs=AOn4CLC_Jda5dMekKZH-buTERFKi4kj5aw" />
      </div>
      <div className='content_title'>
        <img src={ProfileLogo} alt='Profile_Pic' className='profile_icon' />
        <p>
            
        </p>
      </div> */}

    </div>
  )
}

export default containt
